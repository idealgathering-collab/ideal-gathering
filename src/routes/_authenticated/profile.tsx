...
import { INTENTION_OPTIONS, loadMyGatheringPreferences } from "@/lib/gathering-preferences";
...
function ProfilePage() {
  ...
  const original = useRef<{
    profile: ProfileEdit;
    preferences: { intentions: string[]; gathering_types: GatheringType[] };
  } | null>(null);
  ...
  // One initialization per account/load attempt. Card refreshes never refill edits.
  useEffect(() => {
    if (!userId) return;
    let active = true;
    ...
    Promise.all([
      ...
      .then(async ([result, prefs, card]) => {
        ...
        setSocial((row.social_links as SocialLinks) ?? {});
        setIntentions(preferences.intentions);
        setGatheringTypes(preferences.gathering_types);
        setTraitSpark(row.trait_spark);
        setTraitCuriosity(row.trait_curiosity);
        setTraitWarmth(row.trait_warmth);
        setTraitDepth(row.trait_depth);
        setAvatarPath(row.avatar_url);
        setAvatarUrl(avatar);
        setProfileCard(card);
        setFormReady(true);
      })
  ...
  async function saveProfile() {
    ...
    try {
      ...
      const preferences = { intentions, gathering_types: gatheringTypes };
      await saveProfileData(
        changedFields(original.current.profile, profile),
